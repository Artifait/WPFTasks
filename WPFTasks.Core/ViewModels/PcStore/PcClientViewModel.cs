...
{
    public class PcClientViewModel : HintOnMessageInputBoxBaseVm
    {
        ...
        public PcClientViewModel() : base()
        {
            ...
            _pcClient.OnErroreFromServer += error
                => AddMessage("Server", $"Ошибка: [{error.Payload}].");

            _pcClient.OnServerOverloaded += ()
        ...
        private async Task ConnectAsync(string ipAddress, int port)
        {
            try
            {
                await _pcClient.ConnectAsync(ipAddress, port);

                if (_pcClient.IsConnected)
        ...
        private bool CanConnect() => !string.IsNullOrWhiteSpace(IpAddress) && int.TryParse(Port, out _) && !_pcClient.IsConnected;
