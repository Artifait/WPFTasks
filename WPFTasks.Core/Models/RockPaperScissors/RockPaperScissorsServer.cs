...
using WPFTasks.Core.Models.RockPaperScissors.Services;

namespace WPFTasks.Core.Models.RockPaperScissors
{
    public class RockPaperScissorsServer
    {
        // Регистрация MessageBuilder-ов для серверных сообщений
        ...
        public RockPaperScissorsServer(string? userFilePath = null)
        {
            _server.Logger = Logger.LogString;
            _activityService = new(_msgService);
            _gameService = new RPSGameService() { Logger = Logger.LogString };
        ...
        public void SetEndPoint(IPEndPoint endPoint) => _server.SetEndPoint(endPoint);

        public async Task StartServer(CancellationToken token = default)
            => await _server.StartAsync(token);

        public async Task StopServer() => await _server.StopAsync();
